...
class SyntaxAnalyzer:
    ...
    def analyze_file(self, lexical_defs_file, input_file, output_file=None, parser_tables_file=None, debug=False):
        ...
        for line_num, line in enumerate(lines, start=1):
            ...
            # ✅ Mapeamento de tokens para os símbolos da gramática
            mapped_tokens = []
            for token in tokens:
                try:
                    token_clean = token.strip("<>").strip()
                    parts = [x.strip() for x in token_clean.split(",", 1)]

                    if len(parts) == 2:
                        lexeme, token_type = parts
                    else:
                        # Se o token não tiver vírgula, é um símbolo literal
                        lexeme = parts[0]
                        token_type = parts[0]

                    if lexeme in self.lexical_analyzer.symbol_table.reserved_words:
                        mapped_tokens.append(lexeme)
                    elif token_type == "id":
                        mapped_tokens.append("id")
                    elif token_type == "num":
                        mapped_tokens.append("num")
                    elif lexeme in ["if", "else", "while", "(", ")", "{", "}", "<", "=="]:
                        mapped_tokens.append(lexeme)
                    else:
                        mapped_tokens.append(lexeme)
            ...
            if not mapped_tokens or mapped_tokens[-1] != "$":
                mapped_tokens.append("$")

            if not mapped_tokens:
                continue

            # ✅ Garantir que cada sentença termine com um único $
            if mapped_tokens[-1] != "$":
                mapped_tokens.append("$")
